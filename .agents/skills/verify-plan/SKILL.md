...
- This matches the Definition of Done in the repo's root `AGENTS.md` ("First-touch coverage on every user-facing surface"). Keep the language aligned — if AGENTS.md and this skill drift, AGENTS.md is authoritative.
...
- If the plan introduces any new Vue view under `apps/web/src/views/` or substantially changes one, the plan MUST include a step that either:
  - (a) updates the corresponding `.Codex/skills/divinr-<facet>-browser-skill/tests.md` AND adds a spec under `apps/e2e/tests/<facet>/` for the new or changed surface, or
...
- This matches the Definition of Done in the repo's root `AGENTS.md` ("Testing coverage on every user-facing surface"). Keep the language aligned — if AGENTS.md and this skill drift, AGENTS.md is authoritative.
