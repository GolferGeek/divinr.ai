...
Per `AGENTS.md`: admin / debug surfaces may retain domain terminology where it aids maintenance. The vocabulary rule is **RELAXED** for this facet — forbidden words `prediction` / `predicted` / `predictor` / `advice` / `recommendation` are allowed in admin copy. Smoke specs in this project must NOT include the vocabulary check.
