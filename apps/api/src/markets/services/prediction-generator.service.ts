...
export class PredictionGeneratorService {
  ...
  private getThresholdConfig(): ThresholdConfig {
    return {
      minPredictors: parseInt(process.env.MARKETS_MIN_PREDICTORS || '3', 10),
      minAvgRelevance: parseFloat(process.env.MARKETS_MIN_AVG_RELEVANCE || '0.4'),
  ...
  /**
   * Get all active instruments with aggregated predictor stats.
   */
  private async getInstrumentsWithPredictorStats(): Promise<InstrumentWithPredictors[]> {
    const result = await this.db.rawQuery(
      ...
      join prediction.market_predictors mp
        on mp.instrument_id = i.id
        and mp.organization_slug = i.organization_slug
  ...
  private async enqueueRun(
    ...
    const result = await this.db.rawQuery(
      ...
      on conflict on constraint orchestration_runs_unique_queued
        do nothing
      returning id
      `,
    ...
    if (result.error) {
      // If the constraint doesn't exist by that name, try a simpler approach
      if (result.error.message.includes('constraint')) {
        this.logger.debug(`Run already queued for ${instrumentId}`);
        return null;
      }
