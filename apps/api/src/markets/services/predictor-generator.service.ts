...
export class PredictorGeneratorService {
  ...
  private async getActiveInstruments(): Promise<ActiveInstrument[]> {
    const result = await this.db.rawQuery(
      `select id, organization_slug, symbol, name, asset_type
       ...
       order by organization_slug, symbol`,
    );
    if (result.error) {
  ...
  private async scoreArticleForInstrument(
    ...
    // First try keyword matching for quick relevance check
    const quickScore = this.quickKeywordCheck(article, instrument);
    if (quickScore === 0) {
      // Clearly irrelevant — no mention of instrument at all
      await this.upsertPredictor(
        ...
        0,
        'dismissed',
        'No mention of instrument in article',
        'system',
      );
      return { relevanceScore: 0, rationale: 'No mention of instrument', dismissed: true };
    }

    // Use LLM for nuanced scoring
    let relevanceScore = quickScore;
    let rationale = 'Keyword match';
    let dismissed = false;
