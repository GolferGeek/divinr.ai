...
export class MarketsSchemaService {
  ...
  private llmUsageViewsDdl(): string {
    return `
      ...
      create materialized view if not exists prediction.llm_usage_per_source_monthly as
        select
          a.source_id,
          to_char(l."timestamp", 'YYYY-MM') as year_month,
          count(*)::integer as total_calls,
          sum(l.tokens_in)::integer as total_tokens_in,
          sum(l.tokens_out)::integer as total_tokens_out,
          sum(l.cost_cents)::integer as total_cost_cents
  ...
  private outcomeAttributionDdl(): string {
    return `
  ...
  private outcomeAttributionViewsDdl(): string {
    return `
