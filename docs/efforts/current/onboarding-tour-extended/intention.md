...
- **[`testing-team`](../../next/testing-team/intention.md)** (queued in `next/`, sequenced after `ui-vocabulary-and-marketing-refresh`) — Four-layer Chrome testing harness ported from Orchestrator AI: file-based finding lifecycle with dedup hashing, Divinr-specific discover agent + copy-verbatim triage/verify agents, one deep skill per product facet, daily cron pipeline. The first-touch surface inventory in §3 is the authoritative coverage checklist for the deep skills; the testing-team intention sequences *after* this effort so the inventory is stable before skills are written against it.
