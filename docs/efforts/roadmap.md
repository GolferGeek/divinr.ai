...
**Last updated:** 2026-04-20 (user-billing-model Phases 2–7 complete on `effort/user-billing-model` branch; PR pending; ready for `/pr-eval` review & merge)
...
**Status:** Onboarding v2 (extended 5-beat tour + first-touch walkthroughs) shipped. UI vocabulary swept to "analysis/signal" with centralized `<LegalDisclaimer>` variants. Nine-facet testing harness (file-based findings + three agents + Playwright projects) live. `user-billing-model` is effectively complete — Phase 1 merged via PR #68; Phases 2–7 (schema, lifecycle state machine, per-user social opt-outs, pricing + itemized bill, migration + admin read-only view, final verification) all green on branch `effort/user-billing-model`. `stripe-integration` unblocks next.

---

## Current Effort

- **[user-billing-model](current/user-billing-model/intention.md)** — single $50/mo Basic tier, per-item authorship ($20/instrument, $60/analyst), 30-day trial → read-only → purge lifecycle, social opt-outs, migration for grandfathered users, admin read-only view. Clubs are uncoupled from billing (retired concept, master-intention §8). **Phases 1–7 all complete on `effort/user-billing-model`.** Ship PR and run `/pr-eval` to merge + archive.

---

## Recently Shipped

- **[user-billing-model Phases 2–7](current/user-billing-model/plan.md)** (2026-04-20, PR TBD) — single $50/mo Basic tier landed end-to-end. `billing.subscription_events` append-only audit log + `expired_at` / `purge_scheduled_at` columns; `BillingService.{isReadOnly, markExpired, computeLifecycleTransitions, computePurgeCandidates, migrateBackfillSubscriptions}`; `ReadOnlyGuard` + `@SkipReadOnly()` decorator gating every mutating route on `canceled|dormant`; trial seeding threaded into invite + club-code signup. `billing.trial_ended_no_card`, `billing.purge_warning_30d`, `billing.subscription_lifecycle_transition` events emitted on state transitions. Per-user social opt-outs (5 booleans on `authz.users`) threaded through 8 discovery surfaces (club members, tournament roster, tournament leaderboard, club ranking, global leaderboard, messaging search, analyst owner attribution, notification fan-out) via `applyVisibilityFilter`. Itemized `$50 + authored items + BYO platform fee = total` bill view in `BillingTab.vue`; public `/pricing` page; `TrialCountdown` chip + `ReadOnlyBanner` in `DefaultLayout`. Read-only admin view at `/admin/users/:id/billing` (subscription + items + events + preview; no write controls). Idempotent migration backfilled trial rows for every existing user; DB invariant `count(users) == count(subscriptions)` verified on local Supabase. 9 Playwright specs across new `billing` + `admin` projects. Deep testing skill `divinr-billing-browser-skill` registered in the platform index. Four user-visible surfaces wired to first-touch (`billing.trial-countdown`, `billing.read-only-banner`, `billing.bill-overview`, `pricing.overview`, `settings.social-opt-outs`, `admin.user-billing` — inventory now at 111).
