...
var ObservabilityStreamController_1;
Object.defineProperty(exports, "__esModule", { value: true });
exports.ObservabilityStreamController = void 0;
const common_1 = require("@nestjs/common");
const observability_events_service_1 = require("./observability-events.service");
/**
 ...
 */
let ObservabilityStreamController = ObservabilityStreamController_1 = class ObservabilityStreamController {
    observabilityEvents;
    logger = new common_1.Logger(ObservabilityStreamController_1.name);
    constructor(observabilityEvents) {
     ...
     */
    streamEvents(response, filterUserId, filterAgentSlug, filterConversationId) {
        this.logger.debug('🔌 Admin connected to observability stream');
        this.logger.debug(`📋 Filters: userId=${filterUserId || 'none'}, agentSlug=${filterAgentSlug || 'none'}, conversationId=${filterConversationId || 'none'}`);
        // Set SSE headers
        ...
        if (bufferedEvents.length > 0) {
            ...
            const filteredBuffered = bufferedEvents.filter((e) => {
                if (filterConversationId &&
                    ...
                    return false;
                if (filterUserId && e.context.userId !== filterUserId)
                    return false;
            ...
            this.logger.debug(`📦 Sent ${filteredBuffered.length}/${bufferedEvents.length} buffered events (filtered by conversationId: ${filterConversationId || 'none'})`);
        }
        // Subscribe to live observability stream (RxJS-based)
        this.logger.debug(`📡 Subscribing to ObservabilityEventsService. Current buffer size: ${this.observabilityEvents.getSnapshot().length}`);
        const subscription = this.observabilityEvents.events$.subscribe({
            next: (event) => {
                this.logger.debug(`📨 Received event: ${event.hook_event_type} for conversation ${event.context.conversationId || 'none'}`);
                ...
                if (filterConversationId &&
                    ...
                    this.logger.debug(`📨 Filtered out - conversationId mismatch: event=${event.context.conversationId}, filter=${filterConversationId}`);
                    return;
                }
            ...
            error: (error) => {
                this.logger.error(`❌ Subscription error: ${error instanceof Error ? error.message : String(error)}`);
            },
        });
    ...
    writeEvent(response, event) {
        try {
            ...
            this.logger.debug(`✍️ Writing event: ${event.hook_event_type} (${eventJson.length} bytes)`);
     ...
     */
    async getHistory(sinceParam, untilParam, limitParam) {
        const oneHourAgo = Date.now() - 60 * 60 * 1000;
        const since = sinceParam ? parseInt(sinceParam, 10) : oneHourAgo;
        const until = untilParam ? parseInt(untilParam, 10) : undefined;
        const limit = Math.min(Math.max(parseInt(limitParam || '1000', 10), 1), 5000);
        this.logger.log(`📚 Fetching historical events since ${new Date(since).toISOString()}${until ? ` until ${new Date(until).toISOString()}` : ''}, limit ${limit}`);
        const events = await this.observabilityEvents.getHistoricalEvents(since, limit, until);
...
exports.ObservabilityStreamController = ObservabilityStreamController;
__decorate([
    (0, common_1.Get)('stream'),
    __param(0, (0, common_1.Res)()),
    __param(1, (0, common_1.Query)('userId')),
    __param(2, (0, common_1.Query)('agentSlug')),
    __param(3, (0, common_1.Query)('conversationId')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object, String, String, String]),
    __metadata("design:returntype", void 0)
], ObservabilityStreamController.prototype, "streamEvents", null);
__decorate([
...
], ObservabilityStreamController.prototype, "getHistory", null);
exports.ObservabilityStreamController = ObservabilityStreamController = ObservabilityStreamController_1 = __decorate([
    (0, common_1.Controller)('observability'),
    __metadata("design:paramtypes", [observability_events_service_1.ObservabilityEventsService])
