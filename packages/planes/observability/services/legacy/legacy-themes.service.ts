/**
 * LegacyThemesService
 * Merged from apps/observability/server/src/themes/themes.service.ts
 *
...
export class LegacyThemesService {
  ...
  private validateTheme(theme: Partial<Theme>): ThemeValidationError[] {
    ...
    if (!theme.name) {
      ...
      errors.push({
        field: 'name',
        message:
          'Theme name must contain only lowercase letters, numbers, hyphens, and underscores',
        code: 'INVALID_FORMAT',
      });
    }
  ...
  async searchThemes(query: ThemeSearchQuery): Promise<ApiResponse<Theme[]>> {
    try {
  ...
  async deleteThemeById(
    ...
    try {
      ...
      // Only allow deletion by theme author
  ...
  async getThemeStats(): Promise<ApiResponse<ThemeStats>> {
    try {
      ...
      const stats = {
        ...
        totalDownloads: allThemes.reduce(
          (sum, theme) => sum + (theme.downloadCount || 0),
          0,
        ),
        averageRating:
          allThemes.length > 0
